...
const App: React.FC = () => {
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center p-6 sm:p-12 overflow-hidden bg-[#020617] text-slate-100 selection:bg-blue-500/30">
      ...
      {(isCelebration || showPreview) && <Celebration />}
        ...
        {!isCelebration ? (
          <div className="flex flex-col items-center w-full space-y-12 lg:space-y-24">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 sm:gap-16 lg:gap-28 w-full px-4">
              <TimeUnit value={timeLeft.days} label="DAYS" onTrigger={togglePreview} />
              <TimeUnit value={timeLeft.hours} label="HOURS" onTrigger={togglePreview} />
              <TimeUnit value={timeLeft.minutes} label="MINUTES" onTrigger={togglePreview} />
              <TimeUnit value={timeLeft.seconds} label="SECONDS" highlight onTrigger={togglePreview} />
            </div>
            ...
            <div className="max-w-4xl w-full text-center px-6 min-h-[160px] flex items-center justify-center relative">
              {projections.length > 0 ? (
                <div key={currentIdx} className="animate-in fade-in slide-in-from-bottom-8 duration-[1500ms] ease-out">
                  <span className="mono text-[8px] text-emerald-400 block mb-6 tracking-[1em] uppercase opacity-50 font-bold">2026_ODDITY_REPORT</span>
          ...
          <div className="text-center animate-in zoom-in-75 fade-in duration-[4000ms] w-full px-6">
            <h1 className="text-[clamp(6rem,32vw,22rem)] font-black tracking-tighter leading-none text-white drop-shadow-[0_0_120px_rgba(59,130,246,0.6)]">
              2026
            </h1>
            <div className="mt-20 space-y-8">
              <p className="text-3xl sm:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-emerald-400 to-slate-200 tracking-[0.8em] uppercase">
                HELLO FUTURE
              </p>
              <div className="h-px w-64 mx-auto bg-white/5"></div>
              <p className="mono text-[11px] text-slate-500 tracking-[1.2em] uppercase">
      ...
      {showPreview && (
        <div className="fixed bottom-24 mono text-[9px] tracking-[0.5em] text-blue-400/50 uppercase animate-pulse z-20">
          Celebration_Preview_Active
        </div>
      )}
    </div>
  );
...
const TimeUnit: React.FC<TimeUnitProps> = ({ value, label, highlight, onTrigger }) => {
